...
export function testNodes(obj) {
    if (!obj.name) console.error(`Node Does Not Have A Name!`);
    ...
    if (!obj.decision) console.error(`Node Does Not Have A Decision: ${obj.name}`);
    if (!obj.description) console.error(`Node Does Not Have A Description: ${obj.name}`);
